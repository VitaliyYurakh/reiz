...
import {auth, multer, validateImageFile, convertToWebp, requirePermission} from '../middleware';
...
router.get(
    ...
    carController.getConfigurationOptions
);
...
router.get(
    ...
    carController.listDueForService
);
...
router.get(
    ...
    auth,
    requirePermission('cars', 'view'),
    carController.getCityAvailability
);
router.put(
    '/:id/city-availability',
    auth,
    requirePermission('cars', 'full'),
    carController.updateCityAvailability
);
...
router.patch(
    '/:id/maintenance',
    auth,
    requirePermission('service', 'full'),
...
router.patch(
    ...
    validateImageFile,
    convertToWebp,
    carController.addCarPreviewPhoto
);
router.patch(
    '/:id/photo',
    auth,
    ...
    multer,
    validateImageFile,
    carController.updatePhotoCar
);
router.patch(
    '/:id/tariff',
    auth,
    requirePermission('cars', 'full'),
    carController.updateRentalTariff
);
router.patch(
    '/:id/rule',
    auth,
    requirePermission('cars', 'full'),
    carController.updateCountingRule
);
